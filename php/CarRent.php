...
class CarRent
{
    ...
    public function AddDiscount($carId, $description, $percent)
    {
        $car = $this->entityManager->getRepository(Car::class)->findOneBy(["id" => $carId]);
        $query = $this->entityManager->createQueryBuilder()
            ...
            ->setParameter(1, $car)
            ->getQuery();
        $discounts = count($query->getResult());

        if ($car == null || $discounts != 0)
        {
            return false;
        }

        $description = filter_var($description, FILTER_SANITIZE_STRING);
        $percent = filter_var($percent, FILTER_SANITIZE_NUMBER_INT);

        if ($description === false || $percent === false || $percent === '' || $percent < 0 || $percent > 100)
        {
            return false;
        }
        ...
        $this->entityManager->flush();
        $temp = array();
        array_push($temp, $sale->asAssocArray());
        return $temp;
    }
